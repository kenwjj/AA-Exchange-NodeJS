					...
					if (lowestAsk[0].price <= highestBid[0].price) {
						...
						connection.query(query4,[highestBid[0].id], function(err, docs) {
							...
							connection.query(query5,[lowestAsk[0].id], function(err, docs) {
								...
								var query3 = 'Insert IGNORE into matched (stock, bidder, seller, amt, datetime,askid,bidid) values (?,?,?,?,?,?,?);';
								// var query3 = 'IF NOT EXISTS (SELECT bidid,askid FROM matched WHERE bidid = ? AND askid = ?) Insert into matched (stock, bidder, seller, amt, datetime,askid,bidid) values (?,?,?,?,?,?,?);';
